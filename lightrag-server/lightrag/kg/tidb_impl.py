...
class ClientManager:
    ...
    def get_config() -> dict[str, Any]:
        ...
        return {
            ...
            "port": os.environ.get("TIDB_PORT", config.get("tidb", "port", fallback=4000)),
            "user": os.environ.get(
                "TIDB_USER",
    ...
    async def get_by_id(self, id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if self.namespace == NameSpace.VECTOR_STORE_ENTITIES:
                ...
                logger.warning(f"Namespace {self.namespace} not supported for get_by_id")
                return None
    ...
    async def get_by_ids(self, ids: list[str]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if self.namespace == NameSpace.VECTOR_STORE_ENTITIES:
                ...
                logger.warning(f"Namespace {self.namespace} not supported for get_by_ids")
                return []
        ...
        if edge_results:
            ...
            for edge in edge_results:
                ...
                if edge["source_name"] in node_names and edge["target_name"] in node_names:
                    ...
                    edge_properties = {k: v for k, v in edge.items() if k not in ["id", "source_name", "target_name"]}
