...
from .shared_storage import get_storage_lock, get_update_flag, set_all_update_flags
...
class FaissVectorDBStorage(BaseVectorStorage):
    ...
    def __post_init__(self):
        ...
        self._faiss_index_file = os.path.join(self.global_config["working_dir"], f"faiss_index_{self.namespace}.index")
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        if len(embeddings) != len(list_data):
            logger.error(f"Embedding size mismatch. Embeddings: {len(embeddings)}, Data: {len(list_data)}")
            return []
    ...
    async def query(self, query: str, top_k: int, ids: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        logger.info(f"Query: {query}, top_k: {top_k}, threshold: {self.cosine_better_than_threshold}")
    ...
    async def delete(self, ids: list[str]):
        ...
        logger.debug(f"Successfully deleted {len(to_remove)} vectors from {self.namespace}")
    ...
    def _load_faiss_index(self):
        ...
        try:
            ...
            logger.info(f"Faiss index loaded with {self._index.ntotal} vectors from {self._faiss_index_file}")
