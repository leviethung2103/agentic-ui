...
class AGEStorage(BaseGraphStorage):
    ...
    def __init__(self, namespace, global_config, embedding_func):
        ...
        PASSWORD = os.environ["AGE_POSTGRES_PASSWORD"].replace("\\", "\\\\").replace("'", "\\'")
        ...
        for k in record._fields:
            ...
            # this is an attempt to be consistent with neo4j implementation
                ...
                d[k] = (
                    ...
                    edge["label"],  # we don't use decode_graph_label(), since edge label is always "DIRECTED"
    ...
    def _wrap_query(query: str, graph_name: str, **params: str) -> str:
        ...
        if "return" in query.lower():
            ...
            if "*" in [x.strip() for x in fields]:
                raise ValueError("AGE graph does not support 'RETURN *'" + " statements in Cypher queries")

            # get pgsql formatted field names
            fields = [AGEStorage._get_col_name(field, idx) for idx, field in enumerate(fields)]

            # build resulting pgsql relation
            fields_str = ", ".join([field.split(".")[-1] + " agtype" for field in fields])
    ...
    async def get_node_edges(self, source_node_id: str) -> list[tuple[str, str]] | None:
        ...
        for record in results:
            ...
            source_label = source_node["label"] if source_node and source_node["label"] else None
            target_label = connected_node["label"] if connected_node and connected_node["label"] else None
