        ...
        if is_namespace(self.namespace, NameSpace.KV_STORE_LLM_RESPONSE_CACHE):
            ...
            for mode, items in data.items():
                for k, v in items.items():
                    ...
                    update_tasks.append(self._data.update_one({"_id": key}, {"$setOnInsert": v}, upsert=True))
            await asyncio.gather(*update_tasks)
        else:
            update_tasks = []
            for k, v in data.items():
                data[k]["_id"] = k
                update_tasks.append(self._data.update_one({"_id": k}, {"$set": v}, upsert=True))
            await asyncio.gather(*update_tasks)
...
class MongoGraphStorage(BaseGraphStorage):
    ...
    async def initialize(self):
        if self.db is None:
            self.db = await ClientManager.get_client()
            self.collection = await get_or_create_collection(self.db, self._collection_name)
    ...
    async def _graph_lookup(self, start_node_id: str, max_depth: int = None) -> List[dict]:
        """
    ...
    async def upsert_edge(self, source_node_id: str, target_node_id: str, edge_data: dict[str, str]) -> None:
        ...
        await self.collection.update_one({"_id": source_node_id}, {"$pull": {"edges": {"target": target_node_id}}})

        # Insert new edge
        new_edge = {"target": target_node_id}
        new_edge.update(edge_data)
        await self.collection.update_one({"_id": source_node_id}, {"$push": {"edges": new_edge}})
    ...
    async def get_knowledge_graph(self, node_label: str, max_depth: int = 5) -> KnowledgeGraph:
        ...
        try:
            if label == "*":
                    ...
                    if node_id not in seen_nodes:
                        result.nodes.append(
                            KnowledgeGraphNode(
                                ...
                                properties={k: v for k, v in node_doc.items() if k not in ["_id", "edges"]},
                            )
                        )
                            ...
                            if edge_id not in seen_edges:
                                result.edges.append(
                                    KnowledgeGraphEdge(
                                        ...
                                        source=node_id,
                                        target=edge["target"],
                                        properties={k: v for k, v in edge.items() if k not in ["target", "relation"]},
                                    )
                                )
                ...
                pipeline = [
                    {"$match": {"_id": label}},  # Start with nodes having the specified label
                    {
                        "$graphLookup": {
                        ...
                        if node_id not in seen_nodes:
                            result.nodes.append(
                                KnowledgeGraphNode(
                                    ...
                                    properties={
                                        k: v for k, v in connected.items() if k not in ["_id", "edges", "depth"]
                                    },
                                )
                            )
                            seen_nodes.add(node_id)
                                ...
                                if edge_id not in seen_edges:
                                    result.edges.append(
                                        KnowledgeGraphEdge(
                                            ...
                                            target=edge["target"],
                                            properties={
                                                k: v for k, v in edge.items() if k not in ["target", "relation"]
                                            },
                                        )
                                    )
                                    seen_edges.add(edge_id)
    ...
    async def remove_nodes(self, nodes: list[str]) -> None:
        ...
        await self.collection.update_many({}, {"$pull": {"edges": {"target": {"$in": nodes}}}})
    ...
    async def remove_edges(self, edges: list[tuple[str, str]]) -> None:
        ...
        for source, target in edges:
            ...
            update_tasks.append(self.collection.update_one({"_id": source}, {"$pull": {"edges": {"target": target}}}))
...
class MongoVectorDBStorage(BaseVectorStorage):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        for doc in list_data:
            update_tasks.append(self._data.update_one({"_id": doc["_id"]}, {"$set": doc}, upsert=True))
        await asyncio.gather(*update_tasks)
    ...
    async def delete_entity_relation(self, entity_name: str) -> None:
        ...
        try:
            ...
            relations_cursor = self._data.find({"$or": [{"src_id": entity_name}, {"tgt_id": entity_name}]})
